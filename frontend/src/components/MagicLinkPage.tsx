...
export default function MagicLinkPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/auth/magic-link/verify?token=${encodeURIComponent(token)}`)
      ...
      .then(({ ok, data }) => {
        if (ok && data.success) {
          ...
          setStatus('error');
          setMessage(data.error || 'Invalid or expired sign-in link.');
        }
      })
      .catch(() => {
        setStatus('error');
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center p-6">
      <motion.div
        ...
        className="bg-white dark:bg-[#0c0d0f] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-sm p-10 text-center"
        style={{ fontFamily: 'Inter, sans-serif' }}
      >
        {status === 'loading' && (
          <>
            <Loader size={36} className="mx-auto mb-5 text-gray-400 animate-spin" />
            <p className="text-sm text-gray-500 dark:text-gray-500">Signing you in…</p>
          </>
        )}
        ...
        {status === 'error' && (
          <>
            ...
            <motion.button
              onClick={() => { window.location.href = '/'; }}
              className="w-full py-3 rounded-xl border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 text-sm font-medium"
              whileTap={{ scale: 0.98 }}
              whileHover={{ scale: 1.01 }}
            >
              Back to home
